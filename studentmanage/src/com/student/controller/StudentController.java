...
//		view.mainMenu(this);
	...
	public void updateStudent2() {// 선생님 풀이
		...
		boolean resultB = studentDao().updateStudent2(updateStudent);
	...
	public void updateStudent() {// 수정
		...
		String result = studentDao().updateStudent(studentInfo,resultNo);
		mainView().printStudent(result);
	}

	public void deleteStudent() {// 삭제
		int studentNo = mainView().searchStudentByNo();
		Student resultNo = studentDao().searchStudentByNoStudent(studentNo);
		if(resultNo == null) {
			String msg = "조회된 결과가 없습니다";
			mainView().printStudent(msg);
			return;
		}
		String result = studentDao().deleteStudent(resultNo);
		mainView().printStudent(result);
	}

	public void searchStudent() {
		// 1. StudentDao필드에 저장된 학생들을 가져오기
		String sd = studentDao().searchStudent();
		mainView().printStudent(sd);
	}

	public void searchStudentByNo() {
		int studentNo = mainView().searchStudentByNo();
		String result = studentDao().searchStudentByNo(studentNo);
		mainView().printStudent(result);
	}

	public void searchStudentByGrade() {
		int studentGrade = mainView().searchStudentByGrade();
		String result = studentDao().searchStudentByGrade(studentGrade);
		mainView().printStudent(result);
	}

	public void searchStudentByName() {
		String studentName = mainView().searchStudentByName();
		String result = studentDao().searchStudentByName(studentName);
		mainView().printStudent(result);
	}
}
